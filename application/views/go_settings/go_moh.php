...
$(function()
{
	...
	$("#showAllLists").click(function()
	{
		...
		$('#table_container').load('<? echo $base; ?>index.php/go_moh_ce/go_get_list/');
	});
	...
	$("#search_list_button").click(function()
	{
		...
		if (search.length > 2) {
			...
			alert("Por favor, insira ao menos 3 caracteres para iniciar a pesquisa.");
		}
	});
	...
	$('li.go_action_submenu').click(function () {
		var selectedMOH = [];
		$('input:checkbox[id="delMOH[]"]:checked').each(function()
		{
			selectedMOH.push($(this).val());
		});
		...
		{
			alert('Por favor selecione um MOH ID.');
		}
		else
		{
			var s = '';
			...
			if (action == 'delete')
			{
				var what = confirm('Tem certeza que deseja deletar o MOH ID'+s+'?');
				if (what)
				{
...
function addNewMOH()
{
	...
	$('#overlayContent').fadeOut("slow").load('<? echo $base; ?>index.php/go_moh_ce/go_moh_wizard/').fadeIn("slow");
}
...
function delMOH(moh)
{
	var answer = confirm("Tem certeza que deseja deletar "+moh+"?");
	...
	if (answer)
	{
		...
		function(data){
			if (data=="DELETED")
			{
				alert("ENTRADA DE ARQUIVO MOH "+data);
				location.reload();
			}
		});
	}
...
{
	var answer = confirm("Tem certeza que deseja deletar '"+filename+"' da lista de arquivos?");
	
	if (answer)
	{
		$.post("<?=$base?>index.php/go_moh_ce/go_moh_wizard", { mohid: moh, mohfile: filename, action: "delete_file" },
		function(data){
			if (data=="DELETED")
			{
				alert("ENTRADA DE ARQUIVO MOH "+data);
...
function changePage(pagenum)
{
	...
	$('#table_container').load('<? echo $base; ?>index.php/go_moh_ce/go_get_list/'+pagenum+'/'+search);
}
</script>
			...
			<div class="postbox-container" style="width:99%;min-width:1200px;">
				<div class="meta-box-sortables ui-sortable">
					...
					<div id="account_info_status" class="postbox">
						<div class="rightdiv toolTip" id="add_moh" title="Adicionar nova música de espera">Adicionar Nova Música de Espera <img src="<?php echo $base; ?>img/cross.png" style="height:14px; vertical-align:middle;display:none;" />
						</div>
						<div class="hndle">
						...
						<div class="inside">
							...
							<div style="margin:<?php echo (preg_match("/^Windows/",$userOS)) ? "-23px" : "-22px"; ?> 0px -2px -10px;" id="request_tab"><span id="showList" class="tabtoggle menuOn">Música de Espera (MOH) Listagem</span><span id="request" style="display:none;">mostrar lista</span></div>
...
<li class="go_action_submenu" title="Delete Selected" id="delete">Deletar Selecionados</li>
</ul>
</div>
