...
$(function()
{
	...
	$("#search_list_button").click(function()
	{
		...
		if (search.length > 2) {
			$("#table_container").empty().html('<p align="center"><img src="<? echo $base; ?>img/goloading.gif" /></p>');
			$('#table_container').load('<? echo $base; ?>index.php/go_servers_ce/go_update_server_list/search/1/'+search);
		} else {
			alert("Por favor, insira ao menos 3 caracteres para pesquisar");
		}
	});
	...
	$('#search_list').bind("keydown keypress", function(event)
	{
		if (event.type == "keydown") {
			...
			// For ASCII Key Codes
			if ((event.which > 32 && event.which < 46) || (event.which > 46 && event.which < 48) || (event.which > 57 && event.which < 65)
...
{
	var answer = confirm("Tem certeza que deseja deletar "+server+"?");
				...
				<div class="meta-box-sortables ui-sortable">
					...
					<div id="account_info_status" class="postbox">
						<div class="rightdiv toolTip" id="add_carrier" title="Add New Server">
                        	Adicionar Novo Servidor <img src="<?php echo $base; ?>img/cross.png" style="height:14px; vertical-align:middle;display:none;" />
						</div>
						<div class="hndle">
						...
						<div class="inside">
                            ...
                            <div style="margin:<?php echo (preg_match("/^Windows/",$userOS)) ? "-23px" : "-22px"; ?> 0px -2px -10px;" id="request_tab"><span id="showList" class="tabtoggle menuOn">Servidores</span><span id="request" style="display:none;">mostrar lista</span></div>
