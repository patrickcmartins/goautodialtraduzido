...
$(function()
{
	...
	$('li.go_action_submenu').click(function () {
		...
		else
		{
			...
			if (action == 'delete')
			{
				var what = confirm('Tem certeza que deseja deletar a operadora selecionada '+s+'?');
				if (what)
				{
...
<table id="mainTable" class="tablesorter" style="width:100%;" cellpadding=0 cellspacing=0>
	<thead>
		<tr style="font-weight:bold;">
			<th style="white-space:nowrap">&nbsp;ID DA OPERADORA</th>
			<th style="white-space:nowrap">&nbsp;NOME DA OPERADORA</th>
			<th style="white-space:nowrap">&nbsp;IP DO SERVIDOR</th>
			<th>&nbsp;PROTOCOLO</th>
			<th>&nbsp;REGISTRO</th>
			<th>&nbsp;STATUS</th>
			<th>&nbsp;GRUPO</th>
			<th style="width:6%;text-align:center;" nowrap><span style="cursor:pointer;" id="selectAction">&nbsp;AÇÃO &nbsp;<img src="<?php echo $base; ?>img/arrow_down.png" />&nbsp;</span></th>
...
if (count($carriers) > 0) {
   ...
   foreach ($carriers as $list)
   {
	   ...
	   echo "<td style='border-top:#D0D0D0 dashed 1px;' align='center'><span onclick=\"modify('{$list->carrier_id}')\" style='cursor:pointer;margin:5px;' class='toolTip' title='EDITAR OPERADORA<br />{$list->carrier_id}'><img src='{$base}img/edit.png' style='cursor:pointer;width:12px;' /></span>
...
<div class="sippy-info">
    ...
    <table width="100%">
        <tr>
           <td style="width:40%;text-align:center;">Login Web:</td><td><?=$goautodial[0]->username?></td>
        </tr>
        <tr>
           <td style="width:40%;text-align:center;">Senha Web:</td><td><?=$goautodial[0]->web_password?></td>
        </tr>
        <tr>
           <td style="width:40%;text-align:center;">SIP Login:</td><td><?=$goautodial[0]->authname?></td>
        </tr>
        <tr>
           <td style="width:40%;text-align:center;">SIP Senha:</td><td><?=$goautodial[0]->voip_password?></td>
        </tr>
        <tr>
           <td style="width:40%;text-align:center;">VoIP Portal:</td><td><a href="https://billing.voipai.com" target="new" style="color:#7A9E22;">Login Aqui</a></td>
        </tr>
    </table>
</div>
