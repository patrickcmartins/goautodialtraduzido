	...
	$('#saveSettings').click(function()
	{
		...
		if(!isEmpty)
		{
			...
			$.post("<?=$base?>index.php/go_carriers_ce/go_carrier_wizard", { items: items, action: "modify_carrier" },
				...
				{
                                        $(".processing").remove();
					alert("Sucesso!\n\nPor favor, espere até 60 segundos para que as alterações sejam efetuadas.");
...
switch ($type)
{
	...
	default:
?>
<div align="center" style="font-weight:bold; color:#333; font-size:16px;">MODIFICAR OPERADORA: <?php echo "{$carrier_info->carrier_id}"; ?></div>
<br />
...
<table id="test" border=0 cellpadding="3" cellspacing="3" style="width:95%; color:#000; margin-left:auto; margin-right:auto;">
	...
	<tr class="basicConfig">
		...
		<?=form_radio('reg_auth','ipbased',$ipbasedChecked,'id="reg_auth"'); ?> TRUNK &nbsp; 
		<?=form_radio('reg_auth','regstring',$regstrChecked,'id="reg_auth"'); ?> Usuário e senha
		</td>
	</tr>
	...
	<tr>
    	<td><span id="showAdvanceConfig" style="float: left;white-space: nowrap;font-size:11px;color: #7A9E22;cursor: pointer;"><pre style="display: inline;">[+]</pre> CONFIGURAÇÕES AVANÇADAS</span><span id="advance_link" style="cursor:pointer;font-size:9px;display: none;">[ + ADVANCE SETTINGS ]</span><input type="hidden" id="isAdvance" value="0" /></td><td style="text-align:right;"><span id="saveSettings" class="buttons">SALVAR CONFIGURAÇÕES</span><!--<input id="saveSettings" type="submit" value=" SAVE SETTINGS " style="cursor:pointer;" />--></td>
    </tr>
</table>
</form>
<?php
