...
<table id="mainTable" class="tablesorter" style="width:100%;" cellpadding=0 cellspacing=0>
	<thead>
		<tr style="font-weight:bold;">
			...
			<th>&nbsp;MOH NOME</th>
			<th>&nbsp;STATUS</th>
			<th>&nbsp;ORDEM ALEATÓRIA</th>
			<th style="<?php echo $hideFromTenant; ?>">&nbsp;GROUP</th>
			<th colspan="3" style="width:6%;text-align:center;" nowrap><span style="cursor:pointer;" id="selectAction">&nbsp;AÇÃO &nbsp;<img src="<?php echo $base; ?>img/arrow_down.png" />&nbsp;</span></th>
...
if ($moh_list->num_rows() > 0) {
	...
	foreach ($moh_list->result() as $list)
	{
		...
		$status = ($list->active=="Y") ? "ATIVO" : "INATIVO";
		$acolor = ($status=="ATIVO") ? "green" : "#F00";
		$ugroup = ($list->user_group=="---ALL---") ? "ALL USER GROUPS" : $list->user_group;
		$random = ($list->random=="Y") ? "YES" : "NO";
		
		$deleteMOH = "<span onclick=\"delMOH('{$list->moh_id}')\" style='cursor:pointer;' class='toolTip' title='DELETAR MOH ID<br />{$list->moh_id}'>";
		$deleteMOHclass = "";
		$deleteMOHbox = "";
		...
		echo "<td style='border-top:#D0D0D0 dashed 1px;' align='center'><span onclick=\"modify('{$list->moh_id}')\" style='cursor:pointer;' class='toolTip' title='MODIFICAR MOH ID<br />{$list->moh_id}'><img src='{$base}img/edit.png' style='cursor:pointer;width:12px;' /></span></td><td align='center' style='border-top:#D0D0D0 dashed 1px;'>{$deleteMOH}<img src='{$base}img/delete.png' style='width:12px;' {$deleteMOHclass} /></span></td><td align='center' style='border-top:#D0D0D0 dashed 1px;'><span><img src='{$base}img/status_display_i_grayed.png' style='width:12px;' /></span></td>\n";
		echo "<td style='border-top:#D0D0D0 dashed 1px;' align='center'><input type='checkbox' id='delMOH[]' value='{$list->moh_id}' {$deleteMOHbox} /></td>\n";
		echo "</tr>";
	}
} else {
	echo "<tr style=\"background-color:#E0F8E0;\"><td style=\"border-top:#D0D0D0 dashed 1px;font-weight:bold;color:#FF0000;text-align:center;\" colspan=\"9\">Nenhum Registro Encontrado.</td></tr>\n";
}
?>
	</tbody>
</table>
