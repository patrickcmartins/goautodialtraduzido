...
{
	$local_call_times .= '<option value="'.trim($call_times->call_time_id).'">'.$call_times->call_time_id.' - '.$call_times->call_time_name.'</option>';
}
...
<div align="center" style="font-weight:bold; color:#333; font-size:16px;">EDITAR CAMPANHA: <?php echo "$campaign_id - ".$campinfo->campaign_name; ?></div>
<br />
<table id="test" border=0 cellpadding="3" cellspacing="3" style="width:95%; color:#000; margin-left:auto; margin-right:auto;">
	<tr>
    	<td style="text-align:right;" nowrap>Campanha ID:</td><td>&nbsp;<?php echo $campaign_id; ?><input id="campaign_id" type="hidden" value="<?php echo $campaign_id; ?>" /></td>
    </tr>
<?php
if (!$isSurvey)
{
?>
	...
	<tr class="advance_settings" style="display:none;">
    	<td style="text-align:right;" nowrap>Web Form Alvo:</td><td><input type="text" id="web_form_target" class="advanceSettings" value="<?php echo $campinfo->web_form_target; ?>" size="25" maxlength="255" /></td>
    </tr>
	<tr class="advance_settings" style="display:none;">
    	<td style="text-align:right;" nowrap>Permitir Entrada e Mista:</td><td><select id="campaign_allow_inbound" class="advanceSettings"><option>Y</option><option>N</option></select></td>
    </tr>
...
?>
    	<td style="text-align:right;" nowrap>Operadora VoIP:</td><td><?=form_dropdown('dial_prefix',$dial_prefix,$selected_prefix,'id="dial_prefix" style="width:250px;"') ?> <?=form_input('custom_prefix',$campinfo->dial_prefix,'id="custom_prefix" size="12" maxlength="20" '.$show_custom_prefix) ?></td>
    </tr>
<?php
if (!$isSurvey)
{
...
?>
	...
	<tr class="advance_settings" style="display:none;">
    	<td style="text-align:right;" nowrap>Apenas Cálculo Disponível:</td><td><?php echo form_dropdown('available_only_ratio_tally',array('N'=>'NO','Y'=>'YES'),$campinfo->available_only_ratio_tally,'id="available_only_ratio_tally" class="advanceSettings"'); ?></td>
    </tr>
	<tr class="advance_settings" style="display:none;">
...
?>
	<tr>
		...
		<td><select id="active" name="status"><option value="N">INATIVO</option><option value="Y">ATIVO</option></select></td>
	</tr>
	<tr class="advance_settings" style="display:none;">
    	<td style="text-align:right;" nowrap>Apenas Cálculo Disponível:</td><td><?php echo form_dropdown('available_only_ratio_tally',array('N'=>'NO','Y'=>'YES'),$campinfo->available_only_ratio_tally,'id="available_only_ratio_tally" class="advanceSettings"'); ?></td>
    </tr>
...
?>
	...
	<tr>
		<td style="text-align:right;">Número de Linhas:</td>
		<td>
...
?>
	<tr>
    	<td style="text-align:right;" nowrap>Tempo de Chamada Local:</td><td><select id="local_call_time"><?php echo $local_call_times; ?></select></td>
    </tr>
<?php
if ($campinfo->campaign_allow_inbound=="Y")
{
