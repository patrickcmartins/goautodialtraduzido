...
if ($ACTION == 'LogiNCamPaigns')
	{
//agent session
        $stmtagent="SELECT `agent_session_id` , `sess_agent_user` , `sess_agent_phone` , `sess_agent_status` FROM `go_agent_sessions` WHERE `sess_agent_user` = '$user';"; 
        	if ($non_latin > 0) {$rsltagent=mysql_query("SET NAMES 'UTF8'");}
		$rsltagent=mysql_query($stmtagent, $link);
                	if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmtagent,'00004',$user,$server_ip,$session_name,$one_mysql_log);}
                $rowagent=mysql_fetch_row($rsltagent);
                
                $agent_session_id = $rowagent[0];
                $sess_agent_user = $rowagent[1];
                $sess_agent_phone = $rowagent[2];
        ...
        $stmtphone="SELECT `phone_login` FROM `vicidial_users` WHERE user = '$user';";
                if ($non_latin > 0) {$rsltphone=mysql_query("SET NAMES 'UTF8'");}
		$rsltphone=mysql_query($stmtphone, $link);
              	if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmtphone,'00004',$user,$server_ip,$session_name,$one_mysql_log);}
                $rowphone=mysql_fetch_row($rsltphone);
                $thephone = $rowphone[0];
        
        $stmtvcphone="SELECT `sess_agent_user`  FROM `go_agent_sessions` WHERE sess_agent_phone = '$thephone';";
                if ($non_latin > 0) {$rsltvcphone=mysql_query("SET NAMES 'UTF8'");}
		$rsltvcphone=mysql_query($stmtvcphone, $link);
              	if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmtvcphone,'00004',$user,$server_ip,$session_name,$one_mysql_log);}
                $rowvcphone=mysql_fetch_row($rsltvcphone);
	...
	if ( (strlen($user)<1) )
		...
		{
		$stmt="SELECT user_group,user_level,agent_shift_enforcement_override,shift_override_flag,active from vicidial_users where user='$user' and BINARY pass='$pass'";
		if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
		$rslt=mysql_query($stmt, $link);
